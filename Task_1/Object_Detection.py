...
# ################ Computer Vision & Internet of Things Internship ########################
# #########################  Task: 1 Object Detection #####################################
# ############ Task Details: Implement an object detector which identifies ################
# ###############  the classes of the objects in an image or video ########################
# ############################### By Kamran khan ##########################################
...
cap = cv.VideoCapture("E:\Movies\Amsterdam street view Elandsgracht - Centrum.mp4")  # Capturing the webCamp
while True:
    success, frame = cap.read()

    # Converting image to blob format We cannot input the plain image to the network,
    # Yolo net accept only a particular type of image called blob
    blob = cv.dnn.blobFromImage(frame, 1 / 255, (whT, whT), [0, 0, 0], swapRB=True, crop=False)
    ...
    print(f"All layers present in Yolo Net =  {layersNames}")

    # We need only the names of the output layers
    outputLayersNames = net.getUnconnectedOutLayersNames()
    print(f"outputLayersNames = {outputLayersNames}")
